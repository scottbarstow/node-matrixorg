...
Client.prototype = {
  ...
  isLoggedIn: function(){
    console.log("access token: " + this.access_token + " user id: " + this.user_id);
    return this.access_token != undefined && this.user_id != undefined;
  },
  ...
  setRoomName: function(roomId, name, callback){
    return this.sendRoomStateEvent(roomId, "m.room.name", {name:name}, undefined, callback)
  },

  getRoomName: function(roomId, callback){
    return this.getRoomStateEvent(roomId, "m.room.name", callback);
  },

  setRoomTopic: function(roomId, topic, callback){
    return this.sendRoomStateEvent(roomId, "m.room.topic", {topic:topic}, undefined, callback)
  },

  getRoomTopic: function(roomId, callback){
    return this.getRoomStateEvent(roomId, "m.room.topic", callback);
  },

  getRoomStateEvent: function(roomId, eventType, callback){
    var pathParams = {
      $roomId:roomId,
      $eventType: eventType
    }
    var path = encodeUri("/rooms/$roomId/state/$eventType", pathParams);
    return this.makeRequest("get", path, callback);
  },

  sendRoomStateEvent: function(roomId, eventType, data, stateKey, callback){
    var pathParams = {
      $roomId: roomId,
      $eventType: eventType,
      $stateKey: stateKey
    ...
    return this.makeRequest("put", path, data, callback)
  },
